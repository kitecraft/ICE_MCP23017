/*
This library is orginally called 'arduino-mcp23017' and can be found here:
https://github.com/blemasle/arduino-mcp23017
I've added a couple functions from the Adafrunit MCP23017 libary to make
it more convinient.
  uint8_t getLastInterruptPin();
  uint8_t getLastInterruptPinValue();
  (and a couple functions which those two rely on)
The renaming is just to keep this library seperate from those others.
*/
...
enum class Ice_MCP23017Port : uint8_t
{
	A = 0,
	B = 1
};
 ...
 */
enum class Ice_MCP23017InterruptMode : uint8_t
{
 ...
 */
enum class Ice_MCP23017Register : uint8_t
{
...
inline Ice_MCP23017Register operator+(Ice_MCP23017Register a, Ice_MCP23017Port b) {
	return static_cast<Ice_MCP23017Register>(static_cast<uint8_t>(a) + static_cast<uint8_t>(b));
};
...
public:
	 ...
	 */
	Ice_MCP23017(uint8_t address, TwoWire& bus = Wire);
	~Ice_MCP23017();
#ifdef _DEBUG
	void debug();
#endif
	 ...
	 */
	void portMode(Ice_MCP23017Port port, uint8_t directions, uint8_t pullups = 0xFF, uint8_t inverted = 0x00);
	/**
	 ...
	 * See "3.5.10 Port register".
	 */
	void writePort(Ice_MCP23017Port port, uint8_t value);
	/**
	 ...
	 * See "3.5.10 Port register".
	 */
	uint8_t readPort(Ice_MCP23017Port port);
	/**
	 ...
	 */
	void writeRegister(Ice_MCP23017Register reg, uint8_t value);
	/**
	 ...
	 */
	void writeRegister(Ice_MCP23017Register reg, uint8_t portA, uint8_t portB);
	/**
	 * Reads a single register value.
	 */
	uint8_t readRegister(Ice_MCP23017Register reg);
	 ...
	 * will be reversed due to the way the MCP23017 works in Byte mode.
	 */
	void readRegister(Ice_MCP23017Register reg, uint8_t& portA, uint8_t& portB);
	 ...
	 */
	void interruptMode(Ice_MCP23017InterruptMode intMode);
	/**
	 * Configures interrupt registers using an Arduino-like API.
	 * mode can be one of CHANGE, FALLING or RISING.
	 */
	void interrupt(Ice_MCP23017Port port, uint8_t mode);
	/**
	 * Disable interrupts for the specified port.
	 */
	void disableInterrupt(Ice_MCP23017Port port);
	/**
